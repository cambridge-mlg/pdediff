...
class Unet(nn.Module):
    ...
    Args:
        ...
        n_dims (int): Number of spatial dimensions. Defaults to 1.
        time_scale (float): scale of time before time embedding. 
    Note:
    ...
    def __init__(
        ...
        initial_block_multiplier: int = 1,
    ) -> None:
        super().__init__()
        ...
        for i in range(n_resolutions):
            ...
            if i == 0:
                multiplier = initial_block_multiplier
            else:
                multiplier = 1
            # Add `n_blocks`
            for _ in range(int(multiplier*n_blocks)):
                down.append(
                    DownBlock(
                        in_channels,
                        out_channels,
    ...
    def forward(self, x: torch.Tensor, time: torch.Tensor = None, z: torch.Tensor = None):
        ...
        orig_shape = x.shape
        x = x.reshape(x.size(0), -1, *x.shape[-self.n_dims:])  # collapse T,C
        emb = 0
        if time is not None:
        ...
        x_reshaped =  x.reshape(
            *orig_shape[:-self.n_dims - 1], (self.n_output_scalar_components + self.n_output_vector_components * 2)*self.time_future, *orig_shape[-self.n_dims:]
        )
        return x_reshaped