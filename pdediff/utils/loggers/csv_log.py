...
from .utilities import rank_zero_only, rank_zero_warn, save_hparams_to_yaml
...
class CSVLogger(LightningLoggerBase):
    ...
    def log_plot(self, name, plt, step) -> None:
        import PIL
        def save_plt(plt, path):
            if isinstance(plt, PIL.Image.Image):
                plt.save(path)
            else:
        ...
        if isinstance(plt, dict):
            ...
            for plot_name, plot_i in plt.items():
                ...
                save_plt(plot_i, path + f"_{plot_name}.png")
        else:
            # plt.savefig(path + ".png", dpi=300, bbox_inches="tight")
            save_plt(plt, path + ".png")

    def log_pt_results(self, data, name) -> None:
        root_dir = os.path.join(self.log_dir, "sampling_data")
        os.makedirs(root_dir, exist_ok=True)
        path = os.path.join(os.getcwd(), root_dir, name)
